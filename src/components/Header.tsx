...
const Header: React.FC<HeaderProps> = ({ className }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-5 lg:px-7.5 xl:px-10 py-4 lg:py-0">
        {/* Logo */}
        ...
        <nav
          ...
          } fixed top-[4.5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent lg:top-auto`}
        >
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map((item: NavigationItem) => (
              <a
                ...
                href={item.url}
                onClick={handleClick}
                className="block relative font-code text-xl uppercase text-n-1 transition-colors hover:text-color-1 px-4 py-6 md:py-8 lg:text-xs lg:font-semibold lg:leading-5 lg:hover:text-n-1 xl:px-6 lg:py-6"
              >
                {item.title}
              </a>
            ))}
